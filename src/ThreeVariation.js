...
export default function ThreeVariation() {
  ...
  let MarketBanner;
  if (marketState === "US") {
    console.log("this US market!", marketState);
    MarketBanner = () => (
      <div className="outterContainer">
        <div className="imgCont">
          <img src={image} alt={marketState} />
        </div>
        <div className="usBannerContainer">
          <h1>{header}</h1>
          <p>{body} </p>
        </div>
      </div>
    );
  } else if (marketState === "MX") {
    MarketBanner = () => (
      <div className="mexicoOutterContainer">
        <div className="imgCont">
          <img src={image} alt={marketState} />
        </div>
        <div className="mexicoBannerContainer">
          <h1>{header}</h1>
          <p>{body} </p>
        </div>
      </div>
    );
  } else if (marketState === "CA") {
    console.log("this US market!", marketState);
    MarketBanner = () => (
      <div className="outterContainer">
        <div className="canadaBannerContainer">
          <h1>{header}</h1>
          <p>{body} </p>
  ...
  return (
    <div className="App">
      <h1>ThreeVariation Component</h1>
      <NavComponent handleClick={setmarketState} />
      {/* #Usecase -1 */}
      {/* //the approach below is implemented for when the same component varies more than 2x in its style  */}
      {activeData && <MarketBanner />}
    </div>
  );
}
